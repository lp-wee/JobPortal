...
export default function EmployerMessages() {
  ...
  useEffect(() => {
    if (user) getContacts(user.id).then(setContacts);
  }, [user]);

  // Auto-select contact from navigation state (e.g. after status change)
  useEffect(() => {
    const state = location.state as any;
    if (state?.contactUserId && user) {
      setSelectedContact({ userId: state.contactUserId, name: '', lastMessage: '', vacancyId: state.vacancyId });
    }
  }, [location.state, user]);

  useEffect(() => {
    if (user && selectedContact) {
      getConversation(user.id, selectedContact.userId, selectedContact.vacancyId).then(setMessages);
    }
